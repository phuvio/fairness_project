...
# Configure logging
logging.basicConfig(
    filename='fair_model_training_grid_search.log',
    level=logging.INFO,
...
if __name__ == "__main__":
    ...
    lambdas = [0.01, 0.1, 0.25, 0.5, 0.75, 1]
    results = {}
    ...
    tpr_diffs = [results[l]['tpr_diff'] for l in lambda_vals]
    accuracies = [results[l]['accuracy'] for l in lambda_vals]
    ax1 = plt.gca()
    ax1.plot(lambda_vals, tpr_diffs, 'b-o', label='TPR Difference')
    ax1.set_xlabel('Lambda')
    ax1.set_ylabel('TPR Difference', color='b')
    ax1.tick_params(axis='y', labelcolor='b')

    ax2 = ax1.twinx()
    ax2.plot(lambda_vals, accuracies, 'r-s', label='Accuracy')
    ax2.set_ylabel('Accuracy', color='r')
    ax2.tick_params(axis='y', labelcolor='r')

    plt.title('Fairness vs Accuracy Trade-off')
    plt.tight_layout()
    plt.savefig('images/fairness_accuracy_tradeoff_lambda.png')
    plt.show()
