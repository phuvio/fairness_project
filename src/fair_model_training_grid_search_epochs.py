...
# Configure logging
logging.basicConfig(
    filename='fair_model_training_grid_search_epochs.log',
    level=logging.INFO,
...
if __name__ == "__main__":
    ...
    for e in test_epochs:
        ...
        logger.info(f"Number of epochs: {e}")
    ...
    epochs_vals = list(results.keys())
    tpr_diffs = [results[l]['tpr_diff'] for l in epochs_vals]
    accuracies = [results[l]['accuracy'] for l in epochs_vals]
    ax1 = plt.gca()
    ax1.plot(epochs_vals, tpr_diffs, 'b-o', label='TPR Difference')
    ax1.set_xlabel('Number of epochs')
    ax1.set_ylabel('TPR Difference', color='b')
    ax1.tick_params(axis='y', labelcolor='b')

    ax2 = ax1.twinx()
    ax2.plot(epochs_vals, accuracies, 'r-s', label='Accuracy')
    ax2.set_ylabel('Accuracy', color='r')
    ax2.tick_params(axis='y', labelcolor='r')

    plt.title('Fairness vs Accuracy Trade-off')
    plt.tight_layout()
    plt.savefig('images/fairness_accuracy_tradeoff_epochs.png')
    plt.show()
