...
\documentclass[english,twoside,censored,report]{HYthesisML} 
...
\lstset{
    ...
    breaklines=true,            % WRAP LINES AUTOMATICALLY
    breakatwhitespace=true,     % Break at spaces, not in middle of words
    frame=single,               % Optional: adds a frame around code
    columns=fullflexible,       % Helps copy-pasting from PDF
    keepspaces=true
}
...
\title{Project Nr.3: Fairness in Machine Learning}

\author{Petteri Huvio, Luca Maahs}
\date{\today}
...
\supervisors{I don't know yet.}

\keywords{Trustworthy Machine Learning, Fairness, Bias, Mitigation}
\additionalinformation{\translate{\track}}


%% For seminar reports:
%%\additionalinformation{Name of the seminar}
\additionalinformation{Course on Trustworthy Machine Learning}
...
\  Networks  $\rightarrow$ Network algorithms  $\rightarrow$ Control path algorithms $\rightarrow$ Network design and planning algorithms
}}
...
% --- Add this block to HY-CS-main.tex before \begin{document} ---

% We use \newcommand (or \renewcommand) to define these variables directly
% so the title page can read them without crashing.

\makeatletter
\providecommand{\level}{Trustworthy Machine Learning}       % Defines \level
\providecommand{\programme}{Master's Programme in Computer Science} % Defines \programme
\providecommand{\department}{Department of Computer Science} % Defines \department
\providecommand{\university}{University of Helsinki}
\makeatother
